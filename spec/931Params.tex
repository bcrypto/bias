...
СИ должен учитывать указанные в \lstinline{acr_values} предпочтения при выборе 
...
\begin{note*}
Предпочительные уровни гарантий аутентификации могут запрашиваться также через 
...
СИ должен отклонять просроченные объекты (с поправкой на возможную 
рассинхронизацию). 
%
...
При выпуске кода авторизации СИ должен связать его с идентификатором ПС 
...
СИ столкнулся с непредвиденными обстоятельствами, 
которые препятствуют обработке запроса;

\item
% OAuth[Auth]
\lstinline{"temporarily_unavailable"}~---
СИ в настоящее время неспособен обработать
запрос из-за временной перегрузки или технического обслуживания;
...
%
Например, значение 300 означает, что БД действует в течении пяти минут с момента 
формирования ответа.
...
%
Если параметр опущен, то СИ должен определить в документации или другим способом
...
При обработке запроса СИ должен сохранить синхропосылку, переданную в параметре
...
согласованных со СИ во время регистрации. СИ должен проверить, что передан 
...
в запросе на узел Authorization. СИ должен проверить совпадение адресов.
...
обычным образом. При включении параметров в объект JWT их значения должны быть 
повторяться.
...
то СИ должен полагать, что повторяется область соответствующего запроса
авторизации~/ аутентификации.
...
указывающее на использование технологии OIDC. 
...
определяюn запрашиваемые ПС в виде утверждений ресурсы пользователя
